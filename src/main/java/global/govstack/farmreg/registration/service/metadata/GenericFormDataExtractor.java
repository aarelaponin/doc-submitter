...
public class GenericFormDataExtractor {
    ...
    public Map<String, Object> extractAllFormData(String recordId) {
        ...
        try {
            // Get parent form configuration (same as ProcessingAPI uses)
            String parentFormId = metadataService.getParentFormId();
            if (parentFormId == null || parentFormId.isEmpty()) {
                // Use farms_registry which contains the actual data fields
                parentFormId = "farms_registry"; // The ACTUAL table with the data
            }
            LogUtil.info(CLASS_NAME, "Using parent form ID: " + parentFormId);

            // For farmers registry, we need to use the proper table name
            // The actual database table is app_fd_farms_registry
            // FormDataDao expects the table name without the app_fd_ prefix
            String tableName = "farms_registry"; // Without app_fd_ prefix for FormDataDao
