package global.govstack.farmreg.registration.service.metadata;

import org.joget.commons.util.LogUtil;
import global.govstack.farmreg.registration.exception.ConfigurationException;
...
public class YamlMetadataService {
    ...
    /**
     * Default constructor - loads metadata automatically
     */
    public YamlMetadataService() {
        try {
            // Load with default service ID from the file itself
            loadMetadataWithoutValidation();
        } catch (Exception e) {
            LogUtil.warn(CLASS_NAME, "Failed to auto-load metadata: " + e.getMessage());
        }
    }

    /**
     * Load metadata without validating service ID
     */
    private void loadMetadataWithoutValidation() throws ConfigurationException {
        try {
            // Load services.yml
            InputStream inputStream = loadYamlFile(METADATA_FILE);
            Yaml yaml = new Yaml();
            Map<String, Object> yamlData = yaml.load(inputStream);
            ...
            LogUtil.info(CLASS_NAME, "Successfully loaded services.yml metadata");
     ...
     * @throws global.govstack.farmreg.registration.exception.ConfigurationException if config is missing
     */
    public String getForeignKeyColumn(String sectionName) throws global.govstack.farmreg.registration.exception.ConfigurationException {
        ...
        throw new global.govstack.farmreg.registration.exception.ConfigurationException(
